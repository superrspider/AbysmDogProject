...
def send_command2(id_num=127, cmd_mode=0, cmd_data=[], data=[], data_num = 0x08, rtr=0):
    cdata = organize_can_message2(id_num, cmd_mode, cmd_data, data, data_num, rtr)
    write_data(data=can_to_uart(data=cdata, rtr=rtr))
...
def impedance_control(tff=0, id_num=127, pos=0, vel=0, kp=0, kd=0):
    ...
    try:
        ...
        send_command2(id_num=id_num, cmd_mode=1, cmd_data=cmd_data, data=tx_data, data_num=0x08,rtr=0)  # 需要用扩展帧（数据帧）进行发送
        reply_state(id_num=id_num)
    except Exception as e:
